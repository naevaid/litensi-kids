...
|---|---|---|---|---|---|---|
| PK1 | GET | `/paket` (READ Publik list) | ❌ | `status: 'active' \| 'archived' \| 'all'` (default 'active') | `PaketLanggananObject[]` (array all paket urut by monthly_price ASC). <br><br> **⭐ R8 FIX ZERO HARDCODE STATISTIK (HAPUS fallback 342/890/1250):** Sebelum return, endpoint otomatis query `COUNT(*) GROUP BY active_plan` dari tabel `users` (normalisasi slug via `normalizePlanId`) → inject & overwrite field `active_users_count` di setiap row paket dengan COUNT USER ASLI. Free=Total user active_plan=free, dst. Frontend MasterPaketPage cukup pakai `?? 0` + `toLocaleString()` — TIDAK BOLEH ada fallback hardcode angka apapun! Source inject logic: [PaketController L18-L26](file:///d:/litensi-kids/backend/app/Http/Controllers/API/PaketController.php#L18-L26). | [PaketController L13-L28](file:///d:/litensi-kids/backend/app/Http/Controllers/API/PaketController.php#L13-L28) |
...
| PK5 | POST | `/paket` (CREATE Admin) | ✅ | **Identitas:** `name (unique), tagline, description, badge, popular` <br> **Harga:** `monthly_price (int >=0), annual_price (int >=0)` <br> **Status:** `status (active/archived, opsional default active)` <br> **⭐ R6 Kuota AV Harian 1 Kolam:** `batas_menit_av_harian (nullable integer min:0 max:14400, NULL/JIKA TIDAK DIKIRIM = 0)` — 0 = unlimited JUJUR (jika feature one_way_audio/live_camera true) ATAU fitur nonaktif total (jika feature false). <br> **9 Fitur Batasan + Label (all opsional):** <br> • `max_children_devices (int >=0)` + `max_children_devices_label (string)` <br> • `location_tracking (enum dasar/realtime_7d/realtime_30d_sos)` + `location_tracking_label` <br> • `app_restriction (enum terbatas_3/unlimited_jadwal/unlimited_ai)` + `app_restriction_label` <br> • `one_way_audio (bool)` + `one_way_audio_label` <br> • `live_camera (bool)` + `live_camera_label` <br> • `max_geofences (int >=0 / unlimited string)` + `max_geofences_label` <br> • `read_message_notifications (bool)` + `read_message_notifications_label` <br> • `remote_screen_lock (bool)` + `remote_screen_lock_label` <br> **Highlight:** `highlight_features (JSON array string)`. <br><br> **⭐ R8 DEFENSIF FILTER + VALIDATOR 24 FIELD (Lawan SQLSTATE 42S22 & Error 422):** Semua field input difilter via whitelist `PAKET_COLS_EXISTS` (hanya field yang BENAR-BENAR ADA di `SHOW COLUMNS paket_langganan` yang diizinkan — lawan `Unknown column`). Daftar validator 24 field SINKRON dengan fillable PaketLangganan (tidak ada field deprecated `max_children` tanpa `_devices`; tidak ada rule `boolean` untuk enum string). Source: [PaketController L13-L41 (PAKET_COLS_EXISTS + rules array)](file:///d:/litensi-kids/backend/app/Http/Controllers/API/PaketController.php#L13-L41). | HTTP **201**, `{ paket (row setelah save), saved_fields: [array nama field yang benar-benar diupdate] }` | [PaketController L48-L149](file:///d:/litensi-kids/backend/app/Http/Controllers/API/PaketController.php#L48-L149) |
| PK6 | PUT/PATCH | `/paket/{id}` (UPDATE Admin) | ✅ | **SAMA DENGAN PK5 (semua field sometimes).** <br> ⚠️ **KRITIS ATURAN SAVE SYNC (R7 Frontend Bug Fix):** MasterPaketPage handleSavePlanEdit WAJIB call `api.put('/paket/{dbId}', payload_snake_case)` SEBELUM setState + toast! Sebelumnya cuma update state lokal (reload F5 balik ke DB value) → SEKARANG async call PUT + await loadData() re-fetch dari server. <br> **Snake ↔ Camel Reverse Mapper Frontend:** `batas_menit_av_harian ↔ limits.audioVideoMinutes`. **⭐ R8 Frontend Deprecated Mapping Fix:** field deprecated `maxChildren` (camelCase) / `max_children` (snake) sudah DIHAPUS → ganti dengan `maxChildrenDevices` camel / `max_children_devices` snake (sesuai nama kolom DB). Jika frontend masih kirim `max_children` → diabaikan defensive filter PAKET_COLS_EXISTS. Source: [MasterPaketPage L140-L204](file:///d:/litensi-kids/src/components/master/MasterPaketPage.tsx#L140-L204) async handleSavePlanEdit. <br><br> **⭐ R8 Backend Validator 24 Field Sinkron:** Hapus rule `boolean` yang salah untuk kolom `app_restriction` (enum string, bukan bool). Tambah rule `in:` untuk 4 enum string (location_tracking, app_restriction, status, display_target). Validator SINKRON 100% dengan tipe data sebenarnya di tabel `paket_langganan`. Sebelumnya user report PUT /paket/2 → error 422 `validation.boolean` (fixed ✅). | `{ paket }` updated + `saved_fields` | [PaketController L69-L88 + L130-L149](file:///d:/litensi-kids/backend/app/Http/Controllers/API/PaketController.php#L69-L149) |
...
> **⭐ R7 DASHBOARD ATURAN DISPLAY 1 KOLAM AV (Frontend ZERO HARDCODE):**
> - **[DashboardOverviewPage L156-L312](file:///d:/litensi-kids/src/components/dashboard/DashboardOverviewPage.tsx#L156-L312):** Progress card sekarang CUMA 1 KUOTA GABUNG (Audio Listen + Live Camera BERSAMA — TIDAK ADA 2 card terpisah!). Formula: `totalDigunakanAv = usedAudio + usedVideo`. Max limit = `paket_langganan.batas_menit_av_harian` (ditemukan via `paketInfo = paketList.find(...) by name/slug matching zero hardcode`).
> - **LARANGAN KERAS (Zero Tolerance Rule):** Dihapus permanen konstanta `PAKET_LIMITS` yang dulu hardcode nilai fallback Free=30/60, Premium=60/300, dst. SEMUA batas limits 100% diambil dari DB server row `paketInfo` flat fields. JIKA field tidak ada → JUJUR nilai 0 / string kosong (tidak ada default asumsi Premium).
> - Breakdown audio used vs video used TETAP ditampilkan sebagai INFO TEXT KECIL SAJA di bawah progress bar — TIDAK DIPAKAI untuk perhitungan batas kuota.
> - **Progress Bar Gradient Warna:** 0% grey (tidak ada aktivitas); <75% normal (emerald); 75-99% warning (amber); ≥100% HABIS (rose/red gradient) + badge notif "Kuota Habis".
> - Note logic: Jika `maxMinutes === 0` → cek `paketInfo.one_way_audio || live_camera`. Jika feature true = "Unlimited / Belum diset batas". Jika feature false = "Paket saat ini TIDAK MEMILIKI akses Monitor AV. Upgrade paket."

---

### 2.4 MODUL DASHBOARD (Protected per user_id)
> ⚠️ **R7 BESAR KOREKSI CRITICAL BUG (fixpkg-5 audit kuota 240):** Dulu `audio_used_minutes/video_used_minutes` dan `*_max_minutes` DIHITUNG dengan **PROXY ESTIMASI HARDCODE** = `battery_level * usageFactor * planLimits [free: 30/60, premium:60/300, family_pro: 180/1800]` → 100% TIDAK AKURAT (contoh user ID=13 Family Pro seolah terpakai 240/240 MENIT PENUH, padahal data asli tabel kuota_av_monitor_harian CUMA 2 MENIT audio!). SEKARANG 100% AMBIL DARI TABEL ASLI + 3 layer priority aturan paket. TIDAK ADA PROXY SAMA SEKALI.
| # | HTTP Method | Endpoint | Auth | Param | Response res.data | Source |
|---|---|---|---|---|---|---|
| D1 | GET | `/dashboard` (Ringkasan Utama) | ✅ | `user_id (wajib int; jika kosong → return summary 0 JUJUR tanpa fallback)` | Nested object **(100% data ASLI TABEL, TIDAK ADA PROXY)**: <br> `summary: { total_anak, total_device, total_perangkat_online, total_zona_geofence, total_notifikasi, total_notifikasi_unread, video_used_minutes (SUM kuota_av_harian used CURDATE), audio_used_minutes (same), video_max_minutes (1 KOLAM = total_batas_3_layer_priority_semua_anak), audio_max_minutes (SAMA DENGAN video_max_minutes karena 1 KOLAM), _batas_gabungan_all_anak_menit (debug helper sama nilai), fetched_at, has_valid_user? }` <br> `daftar_perangkat: [{id, anak_name, device_name, is_online, battery_level}]` <br> `master_summary {total_user, total_paket_aktif, total_pendapatan_7hari, total_profil_anak}` <br> `notifikasi_terbaru [], log_geofence []` <br><br> **Aturan 3 Layer BATAS KUOTA MAX (ZERO HARDCODE) — SAMA PERSIS dengan MonitorAV AV1 agar Dashboard = halaman Monitor 100% konsisten (R8 critical fix):** <br> ① Tertinggi: `profil_anak.av_minutes_daily_override > 0` (override per anak) <br> ② Menengah: `users.active_plan normalized name match paket_langganan.name → batas_menit_av_harian` <br> ③ Terendah JUJUR: TIDAK ADA paket apapun → 0 (tidak asumsi Premium default) | [Dashboard L18-L192](file:///d:/litensi-kids/backend/app/Http/Controllers/API/DashboardController.php#L18-L192) |
| D2 | GET | `/dashboard/weekly-stats` (Waktu Layar 7 Hari — ZERO HARDCODE BASELINE!) | ✅ | `user_id (wajib; jika kosong → weekly_data[] kosong JUJUR)` | `{ weekly_data: [{day, date, total_minutes, belajar_minutes, hiburan_minutes, log_geofence_count, notifikasi_count, is_weekend, has_activity}][] , total_minutes_7d, avg_daily_hours, period_start, period_end, has_any_activity }` | [Dashboard L200-L270](file:///d:/litensi-kids/backend/app/Http/Controllers/API/DashboardController.php#L200-L270) |
...
> **Aturan Urutan Route**: `/anak/pairing/generate`, `/anak/pairing/confirm`, `/anak/pairing/status` **DITULIS DULU** sebelum wildcard `/anak/{id}`! Lalu route SPESIFIK 3 segment `/anak/{id}/telemetry`, `/anak/{id}/fcm-token`, `/anak/{id}/gps` **HARUS sebelum wildcard GET/PUT/DELETE `/anak/{id}`** (Laravel first-match top-down rule KONVENSI.md L114). Urutan route SESUNGGUHNYA di production [routes/api.php L145-L157](file:///d:/litensi-kids/backend/routes/api.php#L145-L157): L145 AN8 POST telemetry → L149 AN9 POST fcm-token → L154 **AN10 POST gps** → L157 wildcard GET show.
...
| **AN8** | **POST** | `/anak/{id}/telemetry` (UPLOAD SCREEN TIME TELEMETRI DARI ANDROID) | ⚠️ **GATE OWNERSHIP** (Bukan Sanctum) | **MINIMAL SALAH SATU:** <br> `pairing_pin (string 6 digit, exact match DB)` **ATAU** <br> `qr_pairing_code (string LTN-XX-XXXX-SEC exact)` <br> **Dua kosong / salah value → 403.** <br> Param data telemetri opsional: `screen_total_minutes, study_minutes, entertainment_minutes, app_usage_json (JSON string array package name & menit), battery_level, last_active_captured_at (ISO waktu HP)` | `{ uploaded: true, updated_at (ISO server), total_inserted_rows (int), profile_last_active (ISO) }` | [AnakController L430-L498](file:///d:/litensi-kids/backend/app/Http/Controllers/API/AnakController.php#L430-L498) |
| **AN9** | **POST** | `/anak/{id}/fcm-token` (UPLOAD / UPDATE FCM TOKEN PERANGKAT ANAK ANDROID — F3 Production Verified ✅ Notif Android Muncul) | ⚠️ **GATE OWNERSHIP SAMA AN8** | Minimal salah satu `pairing_pin` / `qr_pairing_code` non-empty & cocok DB (salah=403; kosong=403). <br> Body: `fcm_token (string, jika empty → SET NULL = user revoke token)` | `{ fcm_token_length (int, 0 jika revoked), token_revoked (boolean), updated_at (ISO server), last_active (ISO) }` <br> **VERIFIED PRODUCTION**: logcat `Force upload FCM token BERHASIL tokenLen=142` id=12 Diana ✅ | [AnakController L499-L538](file:///d:/litensi-kids/backend/app/Http/Controllers/API/AnakController.php#L499-L538) |
| **AN10** | **POST** | `/anak/{id}/gps` (UPLOAD GPS PERGERAKAN REALTIME DARI ANDROID FusedLocationProviderClient — G2.1 Production Verified ✅ 3/3 Health Scenario PASS Commit `7993722`) | ⚠️ **GATE OWNERSHIP SAMA AN8/AN9 EXACT COPY** | **Validasi Gate:** Minimal `pairing_pin` ATAU `qr_pairing_code` salah satu NON EMPTY & value COCOK DB profil_anak target ID. <br> ❌ Dua kosong → **HTTP 403** `success=false message="Validasi kepemilikan gagal: wajib kirim pairing_pin ATAU qr_pairing_code"` <br> ❌ Salah satu dikirim TIDAK cocok → **HTTP 403** `success=false message="PIN pairing tidak cocok" / "QR Pairing Code tidak cocok"` <br><br> **Body GPS Fields (wajib + opsional):** <br> • `latitude` (float **-90 s/d 90** WGS84 decimal degree) **WAJIB** <br> • `longitude` (float **-180 s/d 180**) **WAJIB** <br> • `captured_at` (string ISO 8601 DATETIME, **WAKTU DARI HP BUKAN SERVER!** — dijadikan snapshot ProfilAnak.last_gps_captured_at) **WAJIB** <br> • `accuracy_meters` (int, nullable, akurasi GPS misal 15 = 15m) <br> • `battery_level` (int 0-100, nullable, level baterai perangkat anak) <br> • `speed_kmh` (double nullable, kecepatan gerak km/jam) <br> • `altitude_m` (double nullable, ketinggian mdpl) <br> • `is_mock_detected` (boolean default false, true jika terdeteksi mock lokasi) | **HTTP 200 Sukses (res.data unwrap):** <br> `{ gps_id (bigint, PK row tabel pergerakan_gps_anak), captured_at (ISO UTC yang tersimpan DB), distance_from_last_known_meters (integer, jarak meter dari previous GPS last known snapshot ProfilAnak; 0 jika first upload ever = prev null), geofence_events_triggered_count (integer 0..N, berapa zona geofence yang trigger ENTER / EXIT event pada upload ini), is_inside_any_active_zone (boolean true = titik GPS saat ini ada di DALAM minimal 1 zona aktif user milik anak), updated_at (ISO server = waktu diterima backend) }` <br><br> **⭐ PRODUCTION VERIFIED 3/3 ID=12 Diana pin=922524:** <br> [A] Empty gate → 403 ✅; [B] PIN 000000 salah → 403 ✅; [C] PIN 922524 benar → 200 gps_id=1 distance=0 ✅ <br> **⭐ Trigger Geofence Otomatis (inside/outside):** Setiap upload GPS, backend loop semua ZonaGeofence user_id anak, hitung Haversine 6371 KM jarak titik vs center zona, banding radius_meters/1000. JIKA state BERUBAH (masuk dari luar / keluar dari dalam) → **INSERT LogGeofence** shape existing child_name STRING (no FK) + **BROADCAST FcmPushService event_type exact snake_case `geofence_enter` / `geofence_exit`** ke perangkat Android (F5 handler siap) + Web Push browser (F4 SW listener siap click_url /monitor). <br> **Graceful Non-Fatal:** Semua geofence block DIBUNGKUS try/catch Throwable → JIKA FCM / DB error → GPS upload TETAP RETURN 200 sukses (history tracking tidak gagal karena notif). | [AnakController L546-L793 — method uploadGpsPergerakan + helper haversineKm private static](file:///d:/litensi-kids/backend/app/Http/Controllers/API/AnakController.php#L546-L793) |
| AN11 | DELETE | `/anak/{id}` (HAPUS + cascade update jumlah anak user) | ✅ | - | Empty data message | [AnakController L795-L815](file:///d:/litensi-kids/backend/app/Http/Controllers/API/AnakController.php#L795-L815) |
...
|---|---|---|---|---|---|---|---|
| AV1 | GET | `/monitor/kuota-hari-ini` (List kuota hari ini semua anak milik user) | ✅ | `user_id (wajib)` + opsional `profil_anak_id?` (hanya anak tertentu) | Array object per anak: `{ profil_anak_id, nama_anak, device_model, tanggal, paket_kuota_menit_harian, digunakan_audio_menit, digunakan_video_menit, total_digunakan_menit, sisa_kuota_menit (-1 = unlimited), persentase_terpakai 0-100, last_mode idle/audio_listen/camera_live, last_started_at, last_stopped_at, last_sesi_id, status_kuota unlimited/normal/hampir_habis/habis }` → UI Monitor render progress bar kuota Listen+Camera gabung. Metadata: `meta.total_anak, meta.total_digunakan_semua_anak_menit` | ✅ SUDAH ADA + firstOrCreate auto upsert row jika hari ini belum ada + **⭐ R8 FIX KRITIS SYNC OTOMATIS REALTIME (setiap read endpoint):** <br> ① **Batas paket selalu dihitung via helper getBatasPaketKuotaAnak() 3 layer priority (SAMA DENGAN DashboardController!)** → disimpan ke `paket_kuota_menit_harian` SETIAP KALI endpoint diakses — TANPA prasyarat `total_digunakan_menit === 0` (sebelumnya HANYA sync jika total=0 → bug row STALE 240 menit ketika user UBAH paket jadi 3 menit dan anak sudah terpakai 4 menit hari ini: AV1 tetap return 240 PALSU, sedangkan Dashboard return 3 BENAR). <br> ② **Recompute SELALU**: `realTotal = audio+video`, `realSisa = batas === 0 ? -1 : max(0,batas-total)`, `realPersen = batas>0 ? min(100,round(total/batas*100)) : 0`. **Response SELALU pakai nilai computed, BUKAN nilai row DB mentah.** Kalau mismatch → auto save row kuota kembali agar sinkron kedepannya (total, sisa, batas disimpan). Persentase tidak disimpan ke DB (tidak ada kolom schema real di 000005 migration). <br> ③ **Defensif: Jangan assign ke kolom TIDAK ADA di schema 000005 (misal `persentase_terpakai`)** → SQLSTATE 42S22 1054 dicegah. Source rule persistensi kolom: [Section 4.14 tabel kuota_av_monitor_harian](#414-kuota_av_monitor_harian--kuotaavmonitorharianphp). | [MonitorAV L77-L160](file:///d:/litensi-kids/backend/app/Http/Controllers/API/MonitorAVController.php#L77-L160) |
...
> **✅ R3 SELESAI: Tab 1 Chat & Permintaan Waktu SUDAH ADA Controller/Migration/Model (smoke test 33/33 PASS). ⚠️ R4 Broadcast Tab 2 MASIH PLAN**
> - Source Controller R3: [ChatInboxController.php L1-L377](file:///d:/litensi-kids/backend/app/Http/Controllers/API/ChatInboxController.php)
> - Migration R3: [000008 pesan_chat](file:///d:/litensi-kids/backend/database/migrations/2026_09_18_000008_buat_tabel_pesan_chat_orangtua_anak_r3.php) + [000009 permintaan_waktu](file:///d:/litensi-kids/backend/database/migrations/2026_09_18_000009_buat_tabel_permintaan_tambah_waktu_layar_r3.php)
> - Route: group prefix('/chat') di [routes/api.php L100-L110](file:///d:/litensi-kids/backend/routes/api.php#L100-L110) — **URUTAN WAJIB sebelum prefix('anak') wildcard**
> - Frontend ChatInboxPage.tsx state lokal SEKARANG SUDAH BISA dihubungkan ke 4 endpoint CH1-CH4 di bawah (hilangkan warning toast state lokal).

| # | HTTP Method | Endpoint | Auth | Param | Response res.data | Status |
|---|---|---|---|---|---|---|
| CH1 | GET | `/chat/threads` (List thread per anak) | ✅ | `user_id (wajib)` | Thread array: `{ thread_id, anak_id, nama_anak, device_model, status_online, battery, last_message_text, last_message_sender, last_time, unread_count, avatar, permintaan_pending_count }`. **ZERO HARDCODE**: battery diambil realtime dari `profil_anak.battery_level` (bukan hardcoded 84%!). | ✅ SUDAH ADA → [getThreads L79-L130](file:///d:/litensi-kids/backend/app/Http/Controllers/API/ChatInboxController.php#L79-L130) |
| CH2 | GET | `/chat/{anakId}/messages?page=1&perPage=50` | ✅ | `user_id (gate ownership via anak.user_id)`, `{anakId} numeric whereNumber` | `{ list: ChatMessage[] (sender:child/parent/system, text, attachments, timestamp, is_read, permintaan_waktu_id), total, page, per_page, has_next, next_cursor, anak: info anak }`. **Side Effect**: Semua pesan child/system di thread otomatis di-mark `is_read=true` saat user buka thread. | ✅ SUDAH ADA → [getMessages L136-L200](file:///d:/litensi-kids/backend/app/Http/Controllers/API/ChatInboxController.php#L136-L200) |
| CH3 | POST | `/chat/{anakId}/send` | ✅ | `user_id (gate jadi sender_user_id auto)`, `text (wajib string max 2000)`, `attachments? nullable JSON array` | `ChatMessage object baru (id, timestamp, sender=parent, text, attachments, is_read, saved_to_db=true)` — data PERSIST ke DB (tidak hilang F5 lagi!). | ✅ SUDAH ADA → [sendMessage L206-L251](file:///d:/litensi-kids/backend/app/Http/Controllers/API/ChatInboxController.php#L206-L251) |
| CH4 | POST | `/chat/{anakId}/grant-waktu-layar` (Quick Grant 5/15/30 menit button) | ✅ | `user_id (gate ownership)`, `durasi_menit: integer min:1 max:1440 (24 jam)`, `catatan? string max 500`, `permintaan_waktu_id? nullable (jika approve permintaan spesifik)`, `catatan_orangtua?` | **ATOMIC TRANSACTION 3 STEP (rollback jika salah 1 gagal)**: (a) Update `permintaan_tambah_waktu_layar` status→approved + approved_by + approved_at (jika ada permintaan_waktu_id); (b) Auto TAMBAH `kuota_av_monitor_harian.paket_kuota_menit_harian` SAMA LOGIC AV4 endpoint `/monitor/kuota/tambah-kuota-manual` (sisa kuota AV bertambah buat hari ini, bukan aturan_aplikasi); (c) INSERT row sender=`system` message thread "Orang tua memberikan tambahan waktu X menit." + catatan. Response: `{ granted_menit, berlaku_sampai_jam, anak_id, nama_anak, permintaan_waktu_id, kuota_sebelum, kuota_sesudah, system_message_id, system_message_text }`. | ✅ SUDAH ADA → [grantWaktuLayar L257-L377](file:///d:/litensi-kids/backend/app/Http/Controllers/API/ChatInboxController.php#L257-L377) |
| BR1 | POST | `/broadcast/kirim` (Kirim broadcast keluarga / kunci layar massal) | ✅ | `user_id (pengirim gate)`, `target: 'all' | profil_anak_id (jika spesifik)`, `urgensi ENUM normal/penting/kunci_layar`, `message TEXT wajib max 2000` | `{ broadcast_id, waktu_kirim, jumlah_target_perangkat, status_kirim: 'queued'/'sent' }` + insert row ke R4 | ❌ BELUM (Tabel R4 Rencana Prioritas 3) |
| BR2 | GET | `/broadcast/riwayat?page=1` | ✅ | `user_id (wajib — hanya tampilkan broadcast yg dikirim user sendiri)` | `{ list: BroadcastHistory[] }` list lama untuk UI "Broadcast Terkirim" | ❌ BELUM (Rencana Prioritas 3) |

---

### 2.16 MODUL HAK AKSES & WALI (Pendamping Co-Parent)
> **✅ R5 MODUL SELESAI (5 Endpoint W1-W5 SUDAH ADA + 2 Migration + 2 Model)**
> Controller: [HakAksesWaliController.php](file:///d:/litensi-kids/backend/app/Http/Controllers/API/HakAksesWaliController.php)
> Migration R5: [000012 undangan_wali_akses](file:///d:/litensi-kids/backend/database/migrations/2026_09_18_000012_buat_tabel_undangan_wali_akses_r5.php) + [000013 permission_wali_per_modul](file:///d:/litensi-kids/backend/database/migrations/2026_09_18_000013_buat_tabel_permission_wali_per_modul_r5.php)
> **Aturan Kritis R5:** 3 role template fixed (JUJUR copy paste state HakAksesTab L30-73), role-1 OrangTua Utama total_users_aktif selalu = 1, role pendamping count HANYA yang status_aktif='aktif' (dihitung real dari DB permission_wali_per_modul). Unique constraint `(parent_user_id, pendamping_user_id)` → 1 keluarga+1 pendamping hanya 1 row relasi (tidak boleh duplicate, ganti status_aktif saja kalau nonaktifkan).

| # | HTTP Method | Endpoint | Auth | Param | Response res.data | Status |
|---|---|---|---|---|---|---|
| W1 | GET | `/hak-akses/roles` | ✅ | `user_id (wajib)` | `{ roles: [ { role_id, role_name, deskripsi, permissions_json (5 boolean canX), total_users_aktif: integer (JUJUR COUNT DB! role-1 selalu=1), is_default } ] }` — Source: [HakAksesWaliController L85-L130](file:///d:/litensi-kids/backend/app/Http/Controllers/API/HakAksesWaliController.php#L85-L130) | ✅ SUDAH ADA |
| W2 | POST | `/hak-akses/undang-kirim` | ✅ | `user_id (PEMILIK KELUARGA — pendamping DILARANG 403!)`, `email_wali (format email valid 422)`, `role_id (role-2/role-3 SAJA role-1 TIDAK BOLEH 422)`, `permission_override_json? (whitelist 5 boolean canX)` | `{ undangan_id, kode_undang_unique (prefix inv-...), status: 'pending_invited', email_terkirim: boolean (false sampai ada SMTP provider), expires_at (+7 hari ISO) }` — Unique pending/accepted per email → 409 Conflict. Source: [HakAksesWaliController L135-L230](file:///d:/litensi-kids/backend/app/Http/Controllers/API/HakAksesWaliController.php#L135-L230) | ✅ SUDAH ADA |
| W3 | POST | `/hak-akses/undangan-terima` | ❓ Public with kode | `kode_invite (wajib 422, 404 kalau tidak ada, 409 kalau status bukan pending, 410 kalau expire >7 hari)`, `user_id_sudah_ada (user ID existing pendamping) OR user_id_baru (new user — nanti integrate register)` | `{ status: 'accepted', pendamping_id, menjadi_wali_untuk_family_user_id, permissions_json (final merge template + override), permission_relasi_id }` — ⭐ DEFENSIF ANTI 1062: cek exist row relasi parent+pendamping → update status_aktif kembali kalau sudah pernah di-nonaktifkan (JANGAN create baru!). Source: [HakAksesWaliController L235-L390](file:///d:/litensi-kids/backend/app/Http/Controllers/API/HakAksesWaliController.php#L235-L390) | ✅ SUDAH ADA |
| W4 | PUT/PATCH | `/hak-akses/pendamping/{pendampingRelasiId}` | ✅ | `user_id (PEMILIK KELUARGA SAJA 403 kalau mismatch)`, `permission_json? (whitelist 5 boolean canX — hanya field yang disebut yang di-override, sisanya ambil sebelum value!)`, `nonaktifkan? boolean (true → set status_aktif='nonaktif' + tanggal_nonaktif=Carbon.now; false → re-aktifkan kembali)` | `{ updated: true, permissions_sebelum, permissions_sesudah, status_aktif }` — **Defensive anti TypeError:** parameter untyped $pendampingRelasiId + is_numeric check 404! Source: [HakAksesWaliController L395-L465](file:///d:/litensi-kids/backend/app/Http/Controllers/API/HakAksesWaliController.php#L395-L465) | ✅ SUDAH ADA |
| W5 | GET | `/hak-akses/daftar-pendamping` | ✅ | `user_id (pemilik keluarga)` | `{ list: [ { relasi_id, pendamping_user_id, nama, email, phone, avatar_url, permissions (5 boolean canX), role_id_awal, tanggal_jadi_awal, tanggal_nonaktif (null kalau aktif), status_aktif } ] }` — Nama/email/avatar dari eager load `users` table ASLI (TIDAK BOLEH fallback hardcode dummy nama!). Source: [HakAksesWaliController L470-L515](file:///d:/litensi-kids/backend/app/Http/Controllers/API/HakAksesWaliController.php#L470-L515) | ✅ SUDAH ADA |
...
| **KUOTA UTAMA 1 KOLAM** | `paket_kuota_menit_harian: integer unsigned default 0` (0 = unlimited) | **⚠️ R6 SUDAH DITERAPKAN PRIORITAS 1! Nilai kolom ini di-set otomatis oleh 3 level priority helper `getBatasPaketKuotaAnak()` di MonitorAVController:** <br> ① (Tertinggi) Jika `profil_anak.av_minutes_daily_override > 0` → override per anak menang. <br> ② (Menengah) Kalau tidak ada override → ambil dari `users.active_plan` → match via `normalizePlanId` → `paket_langganan.batas_menit_av_harian`. <br> ③ (Terendah fallback JUJUR) Kalau user TIDAK punya paket → `0 = unlimited` (tanpa asumsi user paket apa, ZERO HARDCODE!). <br><br> Bisa di-override via AV4 `tambah-kuota-manual` (tambah menit untuk hari ini saja untuk kasus khusus). <br><br> **⭐ R8 FIX KRITIS SYNC REALTIME:** Nilai kolom ini TIDAK BOLEH dianggap "snapshot sekali buat ketika row firstOrCreate". AV1 getKuotaHariIni **SELALU hit ulang via helper & overwrite `paket_kuota_menit_harian` SETIAP KALI endpoint diakses** (meskipun row total sudah ada isinya / anak SUDAH terpakai menit hari ini). Tujuannya: **Jika user UBAH batas_menit_av_harian di halaman Master Paket (PUT /paket/{id})** → Dashboard & halaman Monitor AV HARUS menampilkan NILAI BARU yang SAMA PERSIS (tidak boleh row kuota tetap STALE ke nilai batas lama). |
| **Digunakan (Gabung)** | `digunakan_audio_menit (integer unsigned default 0 — mode Listen)`, `digunakan_video_menit (integer unsigned default 0 — mode Camera)`. **`total_digunakan_menit: integer unsigned index` = SUM audio+video (denormalized agar UI cepat filter paling boros) | Mode audio/video hanya catat masing-masing, total tetap satu kolam! **Ini yang dicek apakah melebihi paket.** | **⭐ R8 Integrity Pattern:** `total_digunakan_menit` di recompute `A+V` setiap AV1 read / AV2 start / AV3 stop / AV4 tambah, lalu auto-save jika mismatch (row DB bisa di-insert manual / data lama dimana total tidak sinkron audio+video → bisa bikin Dashboard kuota PALSU 100%). HINDARI mengambil nilai `total_digunakan_menit` dari row DB mentah tanpa recompute di PHP layer! |
| **Sisa** | `sisa_kuota_menit: integer (-1 = unlimited, 0 = habis, >0 = sisa menit)` | Selalu recompute di setiap read AV1 & write AV2/AV3/AV4 agar konsisten. Formula: `batas === 0 ? -1 : max(0, batas - total_digunakan_recompute)`. |
...
### 4.16 `undangan_wali_akses` → [UndanganWaliAkses.php](file:///d:/litensi-kids/backend/app/Models/UndanganWaliAkses.php)
(R5 BARU DIBUAT PRIORITAS 4. Setiap row = 1 undangan yang dikirim pemilik keluarga ke calon wali/pendamping.)

| Fillable | Tipe Cast | Catatan |
|---|---|---|
| **Explicit table name WAJIB** | `protected $table = 'undangan_wali_akses'` (Laravel pluralization default salah!) | TANPA baris ini → Laravel cari tabel `undangan_wali_akses_s` → SQLSTATE 42S02 Table not found. |
| **FK 2 Relasi User** | `parent_user_id: BIGINT FK cascadeOnDelete users` (PEMILIK KELUARGA yang MENGIRIM undangan → kalau user dihapus, undangan ikut terhapus). **`pendamping_user_id: nullable BIGINT FK nullOnDelete users` (PENDAMPING yang menerima undangan, null sampai ACC W3).** | Index composite `idx_undangan_parent_order (parent_user_id, created_at)` untuk cepat list semua undangan user. Unique composite `(parent_user_id, email_wali, status)` → 1 parent TIDAK BOLEH undang email yang sama 2x ketika masih pending/accepted. |
| **Role & Permission** | `role_id: varchar 50 default 'role-2'`, `role_name: varchar 150 nullable`. **`permission_override_json: array cast nullable` (5 boolean canX whitelist — kalau null pakai template role default).** | Validasi W2: role_id HANYA boleh `role-2 / role-3` (role-1 Ortu utama TIDAK bisa dijadikan role undangan → 422). Permission override di-whitelist 5 key saja, key selain itu di-ignore. |
| **Kode Undangan & Status** | `email_wali: varchar 200 → index`. **`kode_undang_unique: varchar 80 UNIQUE index`** (digunakan W3 endpoint terima undangan). `status: ENUM pending_invited/accepted/declined/expired/cancelled default pending_invited → index`. Validasi W3: status harus `pending_invited` (409 kalau bukan), expired_at < sekarang → mark status=expired + HTTP 410 Gone. | Format kode: `inv-{random12lowercase}-{dechex parentUserId}-{dechex time()}` → globally unique. |
| **Timestamp Workflow** | `invited_at: datetime cast useCurrent`. **`expires_at: datetime cast nullable +7 hari dari invited_at (Auto set W2 kirim undangan).** `accepted_at: datetime cast nullable` (set W3 ACC). `declined_at: datetime cast nullable`. `catatan: text nullable` (alasan tolak dll). | Expire 7 hari sesuai best practice undangan co-parenting (tidak selamanya aktif). |
| **Relasi** | `parentPemilik → BelongsTo User.class 'parent_user_id'`. **`pendampingUser → BelongsTo User.class 'pendamping_user_id'` (nullable).** | Eager loading relasi `pendampingUser` di W5 daftar undangan history. |

---

### 4.17 `permission_wali_per_modul` → [PermissionWaliPerModul.php](file:///d:/litensi-kids/backend/app/Models/PermissionWaliPerModul.php)
(R5 BARU DIBUAT PRIORITAS 4. **Row ini = DATA OTORITAS UTAMA untuk gate kepemilikan** endpoint lain nanti: "apakah user X ini punya HAK AKSES ke data user Y?".)

| Fillable | Tipe Cast | Catatan |
|---|---|---|
| **Explicit table name WAJIB** | `protected $table = 'permission_wali_per_modul'` (Laravel pluralization default salah: menjadi `permission_wali_per_moduls`!). | TANPA baris ini → ERROR 42S02 SQLSTATE Base table not found. |
| **2 FK User + UNIQUE Composite KRITIS** | `parent_user_id: BIGINT FK cascadeOnDelete users` (PEMILIK KELUARGA). **`pendamping_user_id: BIGINT FK cascadeOnDelete users` (PENDAMPING / WALI yang diberi hak).** ⭐ **UNIQUE `uniq_permission_wali_parent_pendamping_unik (parent_user_id, pendamping_user_id)` SAJA** (hanya 1 row per pasangan — tidak include status_aktif, jika tidak ERROR 1062 ketika nonaktifkan 2x!). | Ini KEBENARAN PENTING: 1 keluarga + 1 pendamping = HANYA 1 row hubungan. Kalau mau nonaktifkan tinggal ganti `status_aktif`. Kalau mau invite ulang setelah di-nonaktifkan → update row yang SUDAH ADA set status_aktif kembali ke 'aktif' (DEFENSIF W3 sebelum create, jangan create baru!). |
| **Asal Undangan & Role** | `undangan_asal_id: nullable BIGINT FK undangan_wali_akses.id → nullOnDelete` (row mana yang jadi cikal bakal hubungan ini). `role_id_awal: varchar 50 default 'role-2'` (snapshot role template saat diterima W3). | Bisa null kalau hubungan di-create manual via Master Admin (bukan lewat undangan). |
| **PERMISSION UTAMA 5 BOOLEAN CANX** | ⭐ **`permission_json: JSON cast TIDAK NULLABLE (WAJIB ISI 5 key)`** = shape sesuai [HakAksesTab.tsx L17-L23](file:///d:/litensi-kids/src/components/pengaturan/HakAksesTab.tsx#L17-L23): `{ canLockScreen: bool, canGrantTime: bool, canViewLocation: bool, canEditPin: bool, canBlockApps: bool }`. | Whitelist 5 key ini SELALU di controller sebelum save. Kalau request W4 hanya kirim 2 key → merge dengan nilai SEBELUMNYA (tidak di-reset ke false!). Nanti endpoint lain nanti (contoh: anak/monitor/geofence) bisa baca row ini untuk Otorisasi GATE: "JIKA user adalah PENDAMPING dengan canViewLocation=true → boleh lihat data lokasi keluarga pemilik". |
| **Status Aktif & Tanggal** | `tanggal_jadi_awal: datetime cast useCurrent` (kapan hubungan dimulai). **`tanggal_nonaktif: datetime cast nullable` (set W4 `nonaktifkan=true`).** `status_aktif: ENUM aktif/nonaktif/diblokir default aktif index`. | Count total_users_aktif di W1 roles HANYA yang `status_aktif='aktif'` (nonaktif/diblokir TIDAK dihitung — audit by design). |
| **Audit Admin** | `dibuat_oleh_user_id: nullable BIGINT FK users.id nullOnDelete`. | User ID yang membuat hubungan ini (pendamping sendiri via W3, atau admin sistem manual). |
| **Relasi** | `parentPemilik → BelongsTo User.class 'parent_user_id'`. **`pendampingUser → BelongsTo User.class 'pendamping_user_id'` (eager loading ini = source nama/email W5 daftar pendamping — JANGAN fallback hardcode!).** `undanganAsal → BelongsTo UndanganWaliAkses.class`. | Di User.php ditambahkan 4 relation hasMany R5: `undanganWaliYangDikirim`, `daftarPendampingSaya` (where status aktif), `undanganWaliDiterima`, `sayaJadiWaliUntukKeluarga`. |
...
| R5 ✅ | `undangan_wali_akses` + `permission_wali_per_modul` (2 tabel relasi, migration R5 SUDAH DIBUAT LOKAL & PROD) | ✅ **[R5 SELESAI — 2 Migration + 2 Model + 5 Controller Endpoint W1-W5]** <br> **[Tabel R5a undangan_wali_akses:](file:///d:/litensi-kids/backend/database/migrations/2026_09_18_000012_buat_tabel_undangan_wali_akses_r5.php)** email_wali (200), kode_undang_unique (UNIQUE 80), role_id (varchar 50, default role-2), permission_override_json (JSON nullable), status ENUM('pending_invited','accepted','declined','expired','cancelled') default pending_invited, invited_at timestamp useCurrent, expires_at +7 hari default. FK Custom Cascade: parent_user_id cascadeOnDelete users. FK Custom Nullable: pendamping_user_id nullOnDelete users. <br> **[Tabel R5b permission_wali_per_modul:](file:///d:/litensi-kids/backend/database/migrations/2026_09_18_000013_buat_tabel_permission_wali_per_modul_r5.php)** UNIQUE `uniq_permission_wali_parent_pendamping_unik` **(parent_user_id + pendamping_user_id SAJA — 1 keluarga+1 pendamping=1 row SAJA!)**, role_id_awal (varchar 50), permission_json (JSON TIDAK NULL, 5 boolean canX), tanggal_jadi_awal timestamp useCurrent, tanggal_nonaktif nullable, status_aktif ENUM('aktif','nonaktif','diblokir') default 'aktif'. FK Custom 6 total (Anti InnoDB errno 121): parent_user_id cascade users, pendamping_user_id cascade users, undangan_asal_id nullable nullOnDelete undangan_wali_akses, dibuat_oleh_user_id nullOnDelete users. ⚠️ **[CRITICAL BUG FIX R8-like pattern:]** (1) Model wajib explicit `protected $table` = nama tabel (pluralization Laravel default salah!). (2) Unique composite HANYA parent+pendamping (JANGAN include status_aktif — nanti error 1062 duplicate ketika nonaktifkan data history!). | **Pengaturan Orang Tua > Hak Akses & Wali (HakAksesTab.tsx)** — SEKARANG invite TERSIMPAN permanen ke DB + permission gate check bisa di endpoint lain nanti (where user_id IN (pemilik keluarga OR select parent_user_id from permission_wali_per_modul where pendamping_user_id=X AND status_aktif='aktif') → contoh filter endpoint anak/monitor via user OR wali. | ✅ **R5 SELESAI (Lokal & Prod ✓)** |
