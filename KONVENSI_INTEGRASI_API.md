...
| PK5 | POST | `/paket` (CREATE Admin) | ✅ | **Identitas:** `name (unique), tagline, description, badge, popular` <br> **Harga:** `monthly_price (int >=0), annual_price (int >=0)` <br> **Status:** `status (active/archived, opsional default active)` <br> **⭐ R6 Kuota AV Harian 1 Kolam:** `batas_menit_av_harian (nullable integer min:0 max:14400, NULL/JIKA TIDAK DIKIRIM = 0)` — 0 = unlimited JUJUR (jika feature one_way_audio/live_camera true) ATAU fitur nonaktif total (jika feature false). <br> **9 Fitur Batasan + Label (all opsional):** <br> • `max_children_devices (int >=0)` + `max_children_devices_label (string)` <br> • `location_tracking (enum dasar/realtime_7d/realtime_30d_sos)` + `location_tracking_label` <br> • `app_restriction (enum terbatas_3/unlimited_jadwal/unlimited_ai)` + `app_restriction_label` <br> • `one_way_audio (bool)` + `one_way_audio_label` <br> • `live_camera (bool)` + `live_camera_label` <br> • `max_geofences (int >=0 / unlimited string)` + `max_geofences_label` <br> • `read_message_notifications (bool)` + `read_message_notifications_label` <br> • `remote_screen_lock (bool)` + `remote_screen_lock_label` <br> **Highlight:** `highlight_features (JSON array string)`. | HTTP **201**, `{ paket (row setelah save), saved_fields: [array nama field yang benar-benar diupdate] }` | [PaketController L48-L149](file:///d:/litensi-kids/backend/app/Http/Controllers/API/PaketController.php#L48-L149) |
| PK6 | PUT/PATCH | `/paket/{id}` (UPDATE Admin) | ✅ | **SAMA DENGAN PK5 (semua field sometimes).** <br> ⚠️ **KRITIS ATURAN SAVE SYNC (R7 Frontend Bug Fix):** MasterPaketPage handleSavePlanEdit WAJIB call `api.put('/paket/{dbId}', payload_snake_case)` SEBELUM setState + toast! Sebelumnya cuma update state lokal (reload F5 balik ke DB value) → SEKARANG async call PUT + await loadData() re-fetch dari server. <br> **Snake ↔ Camel Reverse Mapper Frontend:** `batas_menit_av_harian ↔ limits.audioVideoMinutes` (source: [MasterPaketPage L140-L204](file:///d:/litensi-kids/src/components/master/MasterPaketPage.tsx#L140-L204) async handleSavePlanEdit). | `{ paket }` updated + `saved_fields` | [PaketController L69-L88 + L130-L149](file:///d:/litensi-kids/backend/app/Http/Controllers/API/PaketController.php#L69-L149) |
